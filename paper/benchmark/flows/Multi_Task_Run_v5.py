...
# Ensure algos is importable: algos lives under paper/benchmark/algos, so add paper/benchmark to path
...
from algos.CQL_heuristic import CQLHeuristicDiscrete
...
from algos.OneStepRL_heuristic import OneStepRLHeuristicDiscrete
from algos.CPQ_heuristic import CPQHeuristicDiscrete
import torch.nn.functional as F  # for CDT training
from evaluate_unified_v4 import evaluate_unified
from datetime import datetime
from heuristic_bc_base import (
    append_heuristic_actions_to_dataset,
    generate_heuristic_actions,
)
...
def extract_data_from_buffer(buffer):
    ...
    actual_n_agents_list = []  # record actual number of agents per episode
    ...
    for i in range(num_eps):
        ...
        # Record actual n_agents and squeeze extra dims
        actual_n_agents = ep['n_bridges_actual'].cpu().numpy().squeeze()  # [1,1] -> scalar
        ...
        if 'raw_cost' in ep.data.transition_data:
            ...
            # note: actions[0] shape may be (T, n_agents) or (T, n_agents, 1)
    ...
    print(f'Extracted episodes shape: {data.shape}')
...
def compute_episode_scores(data, actions, rewards=None, raw_cost=None, health=None, action_costs=None):
    ...
    Metrics:
      - reward_sum: per-episode reward sum (if rewards available)
      - neg_cost_sum: per-episode negative total cost (higher is better); uses raw_cost or action_costs
      - health_improve: health improvement (if health available), mean over active bridges
    """
...
# =================== Training functions (return training history) ===================
...
def augment_data_with_budgets(data, budgets, budget_mode='provided', log_budget_norm_params=None, episode_budgets=None):
    ...
    Args:
        data: [num_eps, T, n_agents, obs_dim] raw observation data
        budgets: budget info, format depends on budget_mode
        budget_mode: 'provided' or 'uniform'
        log_budget_norm_params: log-budget normalization params
        episode_budgets: per-episode budgets (for uniform mode)
    
    Returns:
        augmented_data: [num_eps, T, n_agents, obs_dim+1] augmented observations
    """
    data = np.asarray(data)
    num_eps, T, n_agents, obs_dim = data.shape
    
    if budget_mode == 'provided':
        # Use provided budgets (already in log form)
        if budgets.ndim == 4 and budgets.shape[-1] == 1:
            budget_arr = budgets  # [num_eps, T, n_agents, 1]
        elif budgets.ndim == 3:
            budget_arr = budgets[..., np.newaxis]  # add last dim
        else:
            raise ValueError(f"Unsupported budgets shape: {budgets.shape}")
            
    elif budget_mode == 'uniform':
        # Compute uniform budget from episode_budgets
        ...
        for ep in range(num_eps):
            if episode_budgets is not None:
                ...
                budget_arr[ep, :, :, 0] = 0.0  # default budget
    else:
        raise ValueError("budget_mode must be 'provided' or 'uniform'")
    
    # Check shape compatibility
    if budget_arr.shape[:3] != data.shape[:3]:
        raise ValueError(f"Budget shape {budget_arr.shape[:3]} does not match data shape {data.shape[:3]}")
...
def train_multitask_iqlcql(
    ...
    if data.shape[-1] == state_dim + 1:
        print(f"Data already includes budget: {data.shape}")
        actual_obs_dim = data.shape[-1]  # use actual obs dim
        model_has_budget = True
    else:
        print(f"Data has no budget: {data.shape}")
    ...
    model.has_budget_in_obs = model_has_budget
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i in range(0, num_eps, batch_size):
            ...
            losses.append(float(loss_dict['loss']))  # ensure Python float
        
        avg_loss = float(np.mean(losses))  # ensure Python float
        print(f"[IQLCQL-MA] Epoch {epoch}, Loss: {avg_loss:.4f}")
        
        # Record training data
        training_history['epochs'].append(int(epoch))  # ensure Python int
        ...
        if ((epoch % eval_interval == 0 or epoch == num_epochs) and
            ...
            print(metrics)
            # Record eval data (ensure type conversion)
            eval_record = {
                'epoch': int(epoch),
...
def train_multitask_offline_cpq(
    ...
    weights[repair_mask] = 100.0      # key hyperparam (suggest 5~20)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i in range(0, N, batch_size):
            ...
            losses.append(float(loss_q))  # ensure Python float
        ...
        if (epoch % eval_interval == 0 or epoch == num_epochs) and test_data is not None:
            # Evaluate: model.act must match evaluate_osrl interface
...
def train_multitask_onestep(
    ...
    def act_wrapper(state, budget=None, deterministic=True, **kwargs):
        ...
        # --- B. Budget concat logic ---
        if budget is not None:
            if not th.is_tensor(budget):
                budget = th.tensor(budget, dtype=th.float32, device=device)
            
            # 1. Ensure State is 2D: (Batch_Size, State_Dim)
            if state.ndim == 1:
                state = state.unsqueeze(0)
            
            # 2. Ensure Budget is 2D: (Batch_Size, 1)
            if budget.ndim == 0:
                # If scalar, make (1, 1)
                budget = budget.view(1, 1)
            elif budget.ndim == 1:
                # If vector (Batch,), make (Batch, 1)
                budget = budget.unsqueeze(1)
            
            # 3. Broadcasting: if State has N>1 rows but Budget is (1,1), all agents share one budget; replicate to match State
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i in range(0, N, batch_size):
            ...
            # Build input with budget
            batch_obs = dataset['obs'][idx]
            batch_bud = dataset['budget'][idx]
            
            # Training dims are usually aligned; unsqueeze for safety
            ...
            # Build batch dict for OneStepRL
            train_batch = {
                'obs': full_obs,
        ...
        # evaluate
        if (epoch % eval_interval == 0 or epoch == num_epochs) and test_data is not None:
            # evaluate_unified calls our patched model.act; act_wrapper handles both scalar and batch budget
            metrics = evaluate_unified(
                model=model,
                data=test_data,
                actions=test_actions,
                budgets=test_log_budgets,  # pass log budgets
                health=test_health,
                action_costs=None,
                raw_cost=test_raw_cost,
                actual_n_agents=actual_n_agents,
    ...
    # Restore original act so model can be saved (pickle cannot serialize local functions)
...
def train_multitask_bc(
    ...
    # Expert trajectory filter (training set only)
    expert_percent=None,
    ...
    """
    # Optional: filter expert trajectories by episode
    if expert_percent is not None:
        try:
            ...
            # If user provides rewards/raw_cost/health, slice them too for consistency
            if rewards is not None:
            ...
            print(f"[MultiTaskBC] Expert filter failed, using full data. Reason: {e}")
    ...
    for ep in range(num_episodes):
        for t in range(T):
            for agent in range(n_agents):
                ...
                # Use log_budgets directly, no denorm
                ...
                obs_list.append(obs)
                budget_list.append(float(budget))  # ensure scalar
                action_list.append(int(act))       # ensure scalar
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i in range(0, obs.shape[0], batch_size):
            ...
            losses.append(float(loss_actor.item()))  # ensure Python float
...
def train_multitask_cql(
    ...
    Args:
        data: [num_episodes, T, n_agents, state_dim]
        actions: [num_episodes, T, n_agents]
        budgets: per-episode budgets [num_episodes]
        log_budgets: log budgets [num_episodes, T, n_agents]
        reward: rewards [num_episodes, T, n_agents]
        ...
        test_budgets: test budgets
        test_raw_cost: test raw cost
        test_health: test health
        actual_n_agents: actual number of agents
        log_budget_norm_params: log budget normalization params
        test_episode_budgets: test episode budgets
        test_log_budgets: test log budgets
    """
    # 1. prepare data
    dataset = prepare_cpq_dataset(data, actions, budgets, log_budgets, reward)
    N = len(dataset['obs'])
...
def train_multitask_cql_heuristic(
    data,
    actions,
    actions_heuristic,
    budgets,
    ...
    if actions_heuristic is None:
        raise ValueError("actions_heuristic is required for cql_heuristic training")

    dataset = prepare_cpq_dataset(data, actions, budgets, log_budgets, reward)
    dataset = append_heuristic_actions_to_dataset(dataset, actions_heuristic, data)
    N = len(dataset['obs'])
    ...
    for k in dataset:
        if k in ['actions', 'actions_heuristic']:
            dtype = th.long
        else:
            dtype = th.float32
        dataset[k] = th.tensor(dataset[k], dtype=dtype, device=device)

    input_dim = state_dim + 1
    model = CQLHeuristicDiscrete(
        state_dim=input_dim,
        n_actions=action_dim,
        hidden_sizes=[256, 256],
        gamma=0.99,
    ...
    print(f"[CQL-Heuristic] Start training with input_dim={input_dim} (State {state_dim} + Budget 1)...")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        losses_total, losses_bellman, losses_cql, losses_heur = [], [], [], []
        ...
        for i in range(0, N, batch_size):
            ...
            train_batch = {
                'obs': full_obs,
                'actions': dataset['actions'][idx],
                'actions_heuristic': dataset['actions_heuristic'][idx],
                'rewards': dataset['rewards'][idx],
                'next_obs': full_next_obs,
                'dones': dataset['dones'][idx]
            }

            loss_total, loss_bellman, loss_cql, loss_heur = model.update(train_batch, optimizer)
            losses_total.append(loss_total)
            losses_bellman.append(loss_bellman)
            losses_cql.append(loss_cql)
            losses_heur.append(loss_heur)

        avg_loss_total = float(np.mean(losses_total))
        avg_loss_bellman = float(np.mean(losses_bellman))
        avg_loss_cql = float(np.mean(losses_cql))
        avg_loss_heur = float(np.mean(losses_heur))

        print(
            f"[CQL-Heuristic] Epoch {epoch}, Total Loss: {avg_loss_total:.4f}, "
            f"Bellman Loss: {avg_loss_bellman:.4f}, CQL Loss: {avg_loss_cql:.4f}, "
            f"Heuristic Loss: {avg_loss_heur:.4f}"
        )
        ...
        training_history['bellman_losses'].append(avg_loss_bellman)
        training_history['cql_losses'].append(avg_loss_cql)
        training_history['heuristic_losses'].append(avg_loss_heur)
        ...
        if (epoch % eval_interval == 0 or epoch == num_epochs) and test_data is not None:
            ...
            print(
                f"[CQL-Heuristic] Eval @ epoch {epoch}: "
                f"behavioral_similarity={metrics['behavioral_similarity_mean']:.4f}, "
                f"cost={metrics['mean_total_cost']:.1f}, vio={metrics['violation_rate_mean']:.3f}"
            )
            eval_record = {
                'epoch': int(epoch),
...
def train_multitask_onestep_heuristic(
    data,
    actions,
    actions_heuristic,
    budgets,
    ...
    if actions_heuristic is None:
        raise ValueError("actions_heuristic is required for onestep_heuristic training")
    ...
    input_dim = state_dim + 1
    model = OneStepRLHeuristicDiscrete(
        state_dim=input_dim,
        n_actions=action_dim,
        device=device,
        hidden_sizes=[256, 256],
    ...
    optimizer_bc = th.optim.Adam(model.bc_net.parameters(), lr=3e-4)
    # Lower learning rate for Q network to prevent explosion
    optimizer_q = th.optim.Adam(model.q_net.parameters(), lr=1e-4)
    ...
    print(f"[OneStepRL-Heuristic] Start training with input_dim={input_dim} (State {state_dim} + Budget 1)...")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        losses_bc, losses_q, losses_heur = [], [], []
        ...
        for i in range(0, N, batch_size):
            ...
            loss_bc, loss_q, loss_heur = model.update(train_batch, optimizer_bc, optimizer_q)
            losses_bc.append(loss_bc)
            losses_q.append(loss_q)
            losses_heur.append(loss_heur)
        ...
        print(
            f"[OneStepRL-Heuristic] Epoch {epoch}, BC Loss: {avg_loss_bc:.4f}, "
            f"Q Loss: {avg_loss_q:.4f}, Heuristic Loss: {avg_loss_heur:.4f}"
        )
        ...
        training_history['q_losses'].append(avg_loss_q)
        training_history['heuristic_losses'].append(avg_loss_heur)
        training_history['losses'].append(avg_loss_bc + avg_loss_q + heuristic_weight * avg_loss_heur)
        ...
        if (epoch % eval_interval == 0 or epoch == num_epochs) and test_data is not None:
            ...
            print(
                f"[OneStepRL-Heuristic] Eval @ epoch {epoch}: "
                f"behavioral_similarity={metrics['behavioral_similarity_mean']:.4f}, "
                f"cost={metrics['mean_total_cost']:.1f}, vio={metrics['violation_rate_mean']:.3f}"
            )
            eval_record = {
...
def train_multitask_cpq_heuristic(
    data,
    actions,
    actions_heuristic,
    budgets,
    ...
    if actions_heuristic is None:
        raise ValueError("actions_heuristic is required for cpq_heuristic training")
    ...
    print(f"[CPQ-Heuristic] Start training...")

    for epoch in range(1, num_epochs + 1):
        perm = th.randperm(N)
        losses_q, losses_qc, losses_heur = [], [], []
        ...
        for i in range(0, N, batch_size):
            ...
            train_batch = {
                ...
                'next_budget': batch_next_bud,
                'costs': dataset['costs'][idx],
                'dones': dataset['dones'][idx]
            }

            loss_q, loss_qc, loss_heur = model.update(train_batch, optimizer_q, optimizer_qc)
            losses_q.append(loss_q)
            losses_qc.append(loss_qc)
            losses_heur.append(loss_heur)

        avg_loss_q = float(np.mean(losses_q))
        avg_loss_qc = float(np.mean(losses_qc))
        avg_loss_heur = float(np.mean(losses_heur))

        print(
            f"[CPQ-Heuristic] Epoch {epoch}, Q Loss: {avg_loss_q:.4f}, "
            f"Qc Loss: {avg_loss_qc:.4f}, Heuristic Loss: {avg_loss_heur:.4f}"
        )
        ...
        training_history['qc_losses'].append(avg_loss_qc)
        training_history['heuristic_losses'].append(avg_loss_heur)
        training_history['losses'].append(avg_loss_q + avg_loss_qc + heuristic_weight * avg_loss_heur)
        ...
        if (epoch % eval_interval == 0 or epoch == num_epochs) and test_data is not None:
            ...
            print(
                f"[CPQ-Heuristic] Eval @ epoch {epoch}: "
                f"behavioral_similarity={metrics['behavioral_similarity_mean']:.4f}, "
                f"cost={metrics['mean_total_cost']:.1f}, vio={metrics['violation_rate_mean']:.3f}"
            )
            eval_record = {
...
def prepare_cdt_dataset(data, actions, rewards, raw_cost, health=None, max_n_agents=None):
    ...
    acts_arr = np.zeros((num_eps, T, max_n_agents, 4), dtype=np.float32)  # assume action_dim=4
    ...
    for ep in range(num_eps):
        for agent in range(min(n_agents, max_n_agents)):
            ...
            # Action data - convert to one-hot
            ...
            if health is not None:
                if health.shape[1] == T:  # health is [T, n_agents]
                    ...
                    health_ep_agent = health[ep, :, agent]  # [T] or [T, 1]
                    if health_ep_agent.ndim > 1:
                        health_ep_agent = health_ep_agent.squeeze(-1)  # ensure [T]
                    health_arr[ep, :T, agent] = health_ep_agent
                    health_arr[ep, T, agent] = health_ep_agent[-1]  # copy last-year health
                elif health.shape[1] == T+1:  # health is [T+1, n_agents]
                    health_ep_agent = health[ep, :, agent]  # [T+1] or [T+1, 1]
                    if health_ep_agent.ndim > 1:
                        health_ep_agent = health_ep_agent.squeeze(-1)  # ensure [T+1]
                    health_arr[ep, :, agent] = health_ep_agent
                else:
                    raise ValueError(f"health time dim incorrect: {health.shape[1]}, expected {T} or {T+1}")
...
def _prepare_cdt_dataset(data, actions, rewards, raw_cost, health=None):
    ...
    for ep in range(N):
        for agent in range(n_agents):
            ...
            # If action shape (T, 1), convert to (T,)
            if acts.ndim > 1:
                acts = acts.squeeze(-1)
            # Action to one-hot, pad to max_adim, postprocess
            action_dim = acts.max() + 1 if acts.max() > 0 else 1
...
def train_multitask_cdt(
    ...
    """
    data: [N, T, state_dim]
    actions: [N, T, action_dim] or [N, T]
    returns_to_go: [N, T]
    costs_to_go: [N, T]
    time_steps: [N, T]
    test_health: [N, T+1] or None
    """
    model = CDT(
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i in range(0, N, batch_size):
            ...
            loss.backward()
            optimizer.step()
            losses.append(float(loss.item()))  # ensure Python float
...
def train_multitask_discrete_bc(
    ...
    actual_n_agents=None, log_budget_norm_params=None, test_episode_budgets=None,
    # Expert trajectory filter toggle (training set only)
    ...
    if expert_percent is not None:
        try:
            ...
            print(f"[DiscreteBC-MA] Expert filter failed, using full data. Reason: {e}")
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for i in range(0, num_eps, batch_size):
            ...
            loss_dict = model.train_step(batch)
            losses.append(float(loss_dict['policy_loss']))  # ensure Python float
        ...
        if (epoch % eval_interval == 0 or epoch == num_epochs) and test_data is not None and env_info is not None:
            # For evaluate_marl: use raw test_data, test_actions, test_log_budgets
...
# Training function for random algorithms
def train_random_baseline(
    ...
    """
    # Create random baseline by algorithm type
    if algo_type == 'osrl':
    ...
    print(f"[Random-{algo_type.upper()}] Random baseline needs no training, evaluating directly...")
    ...
    for epoch in range(eval_interval, num_epochs + 1, eval_interval):
        ...
        if test_data is not None:
            ...
            # Choose evaluation function by algorithm type
            if algo_type == 'osrl':
                metrics = evaluate_unified(
                model=model,
                data=test_data,
                ...
                # Align with OSRL single-agent evaluation
                metrics = evaluate_unified(
                    model=model,
                    data=test_data,
                    actions=test_actions,
...
# Unified test dataset class (used in Multi_Task_Run_v3.py)
...
class UnifiedTestDataset:
    ...
    def __init__(self, test_data, test_actions, test_budgets, test_health, test_raw_cost, 
                 ...
                 test_actual_n_agents, test_data_with_budgets):  # extra param
        ...
        print(f"  - Actual n_agents: {test_actual_n_agents}")
...
# Can pass an algorithm name
def main(input_algo_name=None, device_id=1,seed=42):
    # === config and env ===
    ...
    # print(f"log_budgets.shape = {log_budgets.shape}")
    #exit(0)

    # === build legal_actions ===
    action_costs = {0: 0, 1: 1148.81, 2: 2317.70, 3: 3004.33}
    log_budget_norm_params = env_info['normalization_params']['log_budgets']
    actions_heuristic = None
    if health is not None:
        actions_heuristic = generate_heuristic_actions(
            data=data,
            health=health,
            log_budgets=log_budgets,
            actual_n_agents=actual_n_agents,
            action_costs=action_costs,
            log_budget_norm_params=log_budget_norm_params,
            budget_scale=1.2,
        ...
        print("[WARN] health is None, cql_heuristic will be unavailable.")
    ...
    test_health = test_health[:N]
    test_raw_cost = test_raw_cost[:N]
    test_reward = test_reward[:N]
    test_log_budgets = test_log_budgets[:N]
    ...
    algo_dict = {
        ...
        "onestep_heuristic": lambda: train_multitask_onestep_heuristic(
            data, actions, actions_heuristic, episode_budgets, log_budgets, reward, state_dim, action_dim, device,
            num_epochs=15, batch_size=1024, eval_interval=5,
            heuristic_weight=0.3,
            **test_dataset.get_osrl_data()
        ),
        "multitask_cpq_heuristic": lambda: train_multitask_cpq_heuristic(
            data, actions, actions_heuristic, episode_budgets, log_budgets, reward, state_dim, action_dim, device,
            num_epochs=30, batch_size=1024, eval_interval=5,
            heuristic_weight=0.3,
            **test_dataset.get_osrl_data()
        ...
        "cql_heuristic": lambda: train_multitask_cql_heuristic(
            data, actions, actions_heuristic, episode_budgets, log_budgets, reward, state_dim, action_dim, device,
            num_epochs=30, batch_size=1024, eval_interval=5,
    ...
    if actions_heuristic is None:
        algo_dict.pop("cql_heuristic", None)
        algo_dict.pop("onestep_heuristic", None)
        algo_dict.pop("multitask_cpq_heuristic", None)
        print("[WARN] Skip heuristic algorithms: actions_heuristic not available.")
    ...
    # Can run all algorithms or a specified subset
    ...
    #algorithms_to_run = ["discrete_bc_20"]# 
        # all algorithms
        algorithms_to_run = ["qmix_cql","discrete_bc","iqlcql_marl","onestep","cql","multitask_offline_cpq","cdt","discrete_bc","multitask_bc","random_osrl"]# 
    ...
    # algorithms_to_run = ["random_marl", ...]  # modify this list to select algorithms
    #algorithms_to_run = list(algo_dict.keys())  # run all algorithms
    
    # store results for all algorithms
    all_results = {}
    
    print(f"Training {len(algorithms_to_run)} algorithms: {algorithms_to_run}")
    ...
    for algo_name in algorithms_to_run:
        ...
        try:
            ...
            print(f"\nAlgorithm {algo_name} training done!")
    ...
    print(f"{'Algorithm':<15} {'Accuracy':<10} {'TotalCost':<10} {'Violation':<10} {'HealthImp':<10} {'BudgetUsed':<10} {'BudgetEff':<10}")
    print("-" * 80)
    ...
    print(f"\nAll results saved to metrics_results/ and training_results/")
    print("Use saved data to plot training curves and comparison charts.")
