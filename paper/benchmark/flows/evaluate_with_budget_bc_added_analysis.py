...
def find_latest_files(results_dir):
    ...
    Returns:
        dict: {algorithm_name: latest_file_path}
    """
    ...
    # for每个算法，找到最新的文件（按修改时间）
    latest_files = {}
    for algo_name, files in algorithm_files.items():
        latest_file = max(files, key=os.path.getmtime)
        latest_files[algo_name] = latest_file
        print(f"找到 {algo_name} 的最新文件: {os.path.basename(latest_file)}")
...
def generate_comparison_table(results_dir='paper/benchmark/budget_constrained_results'):
    ...
    # 找到每个算法的最新文件
    latest_files = find_latest_files(results_dir)
    ...
    for algo_name, file_path in latest_files.items():
        try:
            result_data = load_results_from_json(file_path)
            
            if result_data is None:
                print(f"警告: {algo_name} 的文件为空，跳过")
                continue
            ...
            # 提取计算新列所需的字段
            total_health_gain_vs_nothing = result_data.get('total_health_gain_vs_nothing', 0.0)
            mean_step_cost = result_data.get('mean_step_cost', 0.0)
            num_steps = result_data.get('num_steps', 0.0)
            
            # compute单位成本带来的相对于什么也不做的收益
            # avoid除零错误
            ...
            result_row = {
                '算法名称': display_name,
                'budget_utilization': result_data.get('budget_utilization', 0.0),
                'total_health_gain_vs_history': result_data.get('total_health_gain_vs_history', 0.0),
                '单位成本收益': unit_cost_benefit
            }
