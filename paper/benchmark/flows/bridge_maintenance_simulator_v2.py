...
class BridgeMaintenanceSimulator:
    def __init__(self, model_path, algorithm_name, test_data_path, env_info_path=None, action_costs=None, 
        ...
        if action_costs is None:
            self.action_costs = {0: 0, 1: 1148.81, 2: 2317.70, 3: 3004.33}
        else:
    ...
    def load_model(self):
        """加载训练好的模型"""
        try:
            model = th.load(self.model_path, map_location='cuda:0')
            if hasattr(model, 'to'):
                model.to('cuda:0')
            # check并强制覆盖模型内部的 device 属性
            if hasattr(model, 'device'):
                model.device = th.device('cuda:0')
                print(f"模型内部的 'device' 属性已强制更新为: {model.device}")
    ...
    def load_test_data(self):
        """加载测试数据集"""
        try:
            # use与Multi_Task_Run_v3.py相同的方式加载测试数据
    ...
    def apply_budget_multiplier(self):
        ...
        if self.test_data['log_budgets'].ndim == 3:
            self.test_data['log_budgets'] = new_log_budgets[..., np.newaxis]  # restore [T, n_agents, 1] 格式
        else:
    ...
    def apply_budget_allocation_strategy(self):
        ...
        # ✅ 新增：基于健康状态的工程实践策略
            ...
            for t in range(T):
                ...
                # compute需要维修的桥梁数量（预算能覆盖的数量）
                avg_maintenance_cost = total_budget_t / actual_n_agents  # mean维修成本
            ...
            if importance_data is None:
                ...
                for t in range(T):
                    ...
                    # compute综合优先级：重要性权重0.6，健康紧急性权重0.4
            ...
            for t in range(T):
                ...
                if len(preventive_indices) > 0:
                    # at预防性维护范围内，优先健康状态较差的
                    preventive_health = health_t[preventive_indices]
                    preventive_order = np.argsort(preventive_health)  # from差到好
                    ...
                    # if没有处于预防性维护范围的桥梁，回退到critical_first
                    health_order = np.argsort(health_t)
            ...
            for t in range(T):
                ...
                if len(focus_group) > 0:
                    ...
                    focus_order = np.argsort(focus_health)  # from差到好
    ...
    def check_budget_in_obs(self):
        ...
        if hasattr(self.model, 'has_budget_in_obs'):
            ...
            return not self.model.needs_budget_input  # if不需要单独输入预算，说明预算在观察中
        ...
        if self.model_type == 'marl':
            ...
            return False  # default不需要
    
    def augment_data_with_budgets(self, data, log_budgets, budget_mode='provided'):
        """
        Concatenate budget into obs（从Multi_Task_Run_v4.py复制）
        ...
        if budget_mode == 'provided':
            ...
            if log_budgets.ndim == 4 and log_budgets.shape[-1] == 1:
                ...
                budget_arr = log_budgets[..., np.newaxis]  # add最后一维
            else:
    ...
    def create_observation(self, health_state, budget, bridge_age=50, adt=1000, structure_length=100, importance=0.5):
        ...
        # create基础观察向量 [健康值, 桥梁年龄, ADT, 结构长度, 重要性]
        base_obs = np.array([
            normalized_health,
            bridge_age,
            adt,
    ...
    def get_model_action(self, health_state, budget_log, bridge_age=50, adt=1000, structure_length=100, importance=0.5, bridge_idx=0):
        ...
        try:
            ...
            if self.model_type == 'marl' or self.model_type == 'random_marl':
                ...
                budget_tensor = th.tensor([budget_log], dtype=th.float32, device=model_device)  # uselog格式预算
                ...
                # update序列历史
                self.sequence_history['observations'].append(obs)
                self.sequence_history['returns_to_go'].append(1000.0)  # 假设的returns_to_go
                self.sequence_history['costs_to_go'].append(budget_log)    # use当前预算
                self.sequence_history['time_steps'].append(len(self.sequence_history['observations']) - 1)
                
                # 构建序列数据
                seq_len = len(self.sequence_history['observations'])
                obs_seq = np.array(self.sequence_history['observations'])  # [seq_len, state_dim]
                ret_seq = np.array(self.sequence_history['returns_to_go'])  # [seq_len]
                cost_seq = np.array(self.sequence_history['costs_to_go'])   # [seq_len]
                time_seq = np.array(self.sequence_history['time_steps'], dtype=np.int64)  # [seq_len]
                
                # createactions序列 - 确保形状正确
                if len(self.sequence_history['actions']) == 0:
                    # first时间步，创建零动作
                    actions_seq = np.zeros((seq_len, 4), dtype=np.float32)  # [seq_len, action_dim]
                else:
                    # 后续时间步，使用历史动作
                    actions_seq = np.array(self.sequence_history['actions'])  # [seq_len, action_dim]
                    # ensureactions_seq的形状正确
                    if actions_seq.shape[0] != seq_len:
                        # if长度不匹配，调整到正确长度
                        if actions_seq.shape[0] < seq_len:
                            # if历史动作不够，用零动作填充
                            padding = np.zeros((seq_len - actions_seq.shape[0], 4), dtype=np.float32)
                            actions_seq = np.vstack([actions_seq, padding])
                        else:
                            # if历史动作太多，截取
                            actions_seq = actions_seq[:seq_len]
                
                # addbatch维度 - CDT模型expected [batch_size, seq_len, ...]
                obs_seq = obs_seq.reshape(1, seq_len, -1)  # [1, seq_len, state_dim]
                actions_seq = actions_seq.reshape(1, seq_len, -1)  # [1, seq_len, action_dim]
                ret_seq = ret_seq.reshape(1, seq_len)  # [1, seq_len]
                cost_seq = cost_seq.reshape(1, seq_len)  # [1, seq_len]
                time_seq = time_seq.reshape(1, seq_len)  # [1, seq_len]
                
                # will数据移动到模型设备
                obs_tensor = th.tensor(obs_seq, dtype=th.float32, device=model_device)
                actions_tensor = th.tensor(actions_seq, dtype=th.float32, device=model_device)
                ret_tensor = th.tensor(ret_seq, dtype=th.float32, device=model_device)
                cost_tensor = th.tensor(cost_seq, dtype=th.float32, device=model_device)
                time_tensor = th.tensor(time_seq, dtype=th.long, device=model_device)
                

                # limit序列长度，避免内存问题
                max_seq_len = 10  # limit最大序列长度
                if len(self.sequence_history['observations']) > max_seq_len:
                    # 只保留最近的max_seq_len个时间步
                    self.sequence_history['observations'] = self.sequence_history['observations'][-max_seq_len:]
                    self.sequence_history['returns_to_go'] = self.sequence_history['returns_to_go'][-max_seq_len:]
                    self.sequence_history['costs_to_go'] = self.sequence_history['costs_to_go'][-max_seq_len:]
                    self.sequence_history['time_steps'] = self.sequence_history['time_steps'][-max_seq_len:]
                    self.sequence_history['actions'] = self.sequence_history['actions'][-max_seq_len:]

                #print(f"CDT输入形状 - obs: {obs_tensor.shape}, actions: {actions_tensor.shape}, ret: {ret_tensor.shape}, cost: {cost_tensor.shape}, time: {time_tensor.shape}")
                ...
                with th.no_grad():
                    pred = self.model.act(obs_tensor, actions_tensor, ret_tensor, cost_tensor, time_tensor)
                
                
                
                # CDT模型返回的是numpy数组，需要正确处理
                if isinstance(pred, np.ndarray):
                    # CDT返回的是动作logits，使用argmax选择动作
                    if pred.ndim == 2 and pred.shape[1] == 4:
                        # shape为 (1, 4)，取argmax
                        action = int(np.argmax(pred[0]))
                    elif pred.ndim == 1 and pred.shape[0] == 4:
                        # shape为 (4,)，直接取argmax
                        action = int(np.argmax(pred))
                    else:
                        # 其他情况，取第一个元素
                        action = int(pred[0]) if pred.ndim > 0 else int(pred)
                ...
                self.sequence_history['actions'].append(action_one_hot)
                
                return action
                
        except Exception as e:
            print(f"模型预测失败: {e}+{self.model_type}")
            import traceback
            traceback.print_exc()
            return 0
    ...
    def run_simulation(self, n_years=100, base_budget_per_bridge=10000):
        ...
        for year in range(1, n_years + 1):
            ...
            year_data = {
                'year': year,
                'episode_budget': float(test_episode_budget),  # use数据集中的episode预算
                'actions': [],
                'costs': [],
                'budgets_used': [],  # record实际使用的预算
            ...
            for bridge_idx in range(n_agents):
                ...
                # ✅ 直接从测试数据中获取预算（log格式，已按归一化/乘子处理）
                budget_log = float(test_log_budgets[data_idx, bridge_idx])
                
                # get其他特征：注意 test_data 中的 obs 已经在生成 buffer 时归一化过一次
                # here先按 env_info 的 policy_obs 参数反归一化回“原始量纲”，
                # then交给 create_observation 去做一次统一归一化，避免 double-normalization。
                test_obs = test_data[data_idx, bridge_idx]  # [state_dim]，当前是已归一化的观测
                if 'policy_obs' in self.norm_params:
                    mean_vec = np.array(self.norm_params['policy_obs']['mean'])
                    std_vec = np.array(self.norm_params['policy_obs']['std'])
                    raw_obs = test_obs * (std_vec + 1e-8) + mean_vec
                else:
                    raw_obs = test_obs
                
                bridge_age = float(raw_obs[1]) if len(raw_obs) > 1 else 50.0
                adt = float(raw_obs[2]) if len(raw_obs) > 2 else 1000.0
                structure_length = float(raw_obs[3]) if len(raw_obs) > 3 else 100.0
                importance = float(raw_obs[4]) if len(raw_obs) > 4 else 0.5
                
                # for于超出测试数据范围的年份，适当调整桥梁年龄
                if year > T:
                    bridge_age += (year - T)
                
                # ✅ 关键修复：使用当前更新后的健康状态和正确的预算调用模型
                action = self.get_model_action(
                    current_health,      # use当前年份的健康状态
                    budget_log,          # use数据集中的log格式预算
                    bridge_age, 
                    adt, 
                    structure_length, 
                    importance, 
        ...
        print("仿真完成!")
        # ✅ 增强返回值，添加更多统计信息
        enhanced_results = self.simulation_results.copy()
        # add汇总统计
        enhanced_results['summary_statistics'] = {
            'total_simulation_years': n_years,
            'total_bridges': n_agents,
            'active_bridges': actual_n_agents,
            'initial_health_level': self.initial_health_level,
            'budget_multiplier': self.budget_multiplier,
            'budget_allocation_strategy': self.budget_allocation_strategy,
            'algorithm_name': self.algorithm_name,
            
            # 成本统计
            'total_cost': float(np.sum(self.simulation_results['total_costs'])),
            'average_annual_cost': float(np.mean(self.simulation_results['total_costs'])),
            'max_annual_cost': float(np.max(self.simulation_results['total_costs'])),
            'min_annual_cost': float(np.min(self.simulation_results['total_costs'])),
            'cost_std': float(np.std(self.simulation_results['total_costs'])),
            
            # 健康状态统计
            'initial_avg_health': float(np.mean(health_histories[0])),
            'final_avg_health': float(np.mean(health_histories[-1])),
            'avg_health_over_time': float(np.mean([np.mean(health) for health in self.simulation_results['health_histories']])),
            'health_improvement': float(np.mean(health_histories[-1]) - np.mean(health_histories[0])),
            
            # 最终健康状态分布
            'final_health_distribution': {
                'excellent_ratio': float(np.sum((health_histories[-1] >= 8) & (health_histories[-1] <= 9)) / len(health_histories[-1])),
                'good_ratio': float(np.sum((health_histories[-1] >= 6) & (health_histories[-1] <= 7)) / len(health_histories[-1])),
                'fair_ratio': float(np.sum((health_histories[-1] >= 4) & (health_histories[-1] <= 5)) / len(health_histories[-1])),
                'poor_ratio': float(np.sum((health_histories[-1] >= 0) & (health_histories[-1] <= 3)) / len(health_histories[-1]))
            },
            
            # action统计
            'action_statistics': self._calculate_action_statistics(),
            
            # budget使用统计
            'budget_statistics': self._calculate_budget_statistics(),
            
            # 效率指标
            'efficiency_metrics': self._calculate_efficiency_metrics(health_histories),
    ...
    def _calculate_action_statistics(self):
        """计算动作使用统计"""
        all_actions = []
        for year_actions in self.simulation_results['action_distributions']:
            for action, count_ratio in year_actions.items():
                # 假设每年有相同数量的决策
                estimated_count = int(count_ratio * self.test_data['actual_n_agents'])
                all_actions.extend([action] * estimated_count)
        
        if not all_actions:
            return {'no_action_ratio': 0, 'maintenance_ratio': 0, 'major_repair_ratio': 0}
        
        total_actions = len(all_actions)
        return {
            'no_action_ratio': float(all_actions.count(0) / total_actions),
            'minor_repair_ratio': float(all_actions.count(1) / total_actions),
            'medium_repair_ratio': float(all_actions.count(2) / total_actions),
            'major_repair_ratio': float(all_actions.count(3) / total_actions),
            'total_decisions': total_actions,
            'action_entropy': self._calculate_entropy([all_actions.count(i)/total_actions for i in range(4)])
        }

    def _calculate_budget_statistics(self):
        """计算预算使用统计"""
        # from测试数据获取预算信息
        test_log_budgets = self.test_data['log_budgets']
        if test_log_budgets.ndim == 3:
            test_log_budgets = test_log_budgets.squeeze(-1)
        
        # 反归一化预算
        if 'normalization_params' in self.env_info and 'log_budgets' in self.env_info['normalization_params']:
            params = self.env_info['normalization_params']['log_budgets']
            raw_log_budgets = test_log_budgets * params['std'] + params['mean']
            raw_budgets = np.expm1(raw_log_budgets)
        else:
            raw_budgets = test_log_budgets
        
        actual_n_agents = self.test_data['actual_n_agents']
        active_budgets = raw_budgets[:, :actual_n_agents].flatten()
        
        return {
            'total_budget_allocated': float(np.sum(active_budgets)),
            'average_bridge_budget': float(np.mean(active_budgets)),
            'budget_std': float(np.std(active_budgets)),
            'max_bridge_budget': float(np.max(active_budgets)),
            'min_bridge_budget': float(np.min(active_budgets)),
            'budget_sparsity': float(np.sum(active_budgets == 0) / len(active_budgets)),
            'episode_total_budget': float(self.test_data['budgets'])
        }

    def _calculate_efficiency_metrics(self, health_histories):
        """计算效率指标"""
        total_cost = np.sum(self.simulation_results['total_costs'])
        health_improvement = np.mean(health_histories[-1]) - np.mean(health_histories[0])
        avg_health = np.mean([np.mean(health) for health in self.simulation_results['health_histories']])
        
        # 健康状态维持成本效率
        health_maintenance_efficiency = avg_health / (total_cost / 1000000) if total_cost > 0 else 0
        
        # 健康改善成本效率
        health_improvement_efficiency = health_improvement / (total_cost / 1000000) if total_cost > 0 and health_improvement != 0 else 0
        
        # compute健康状态变异系数（稳定性指标）
        health_variations = [np.std(health) for health in self.simulation_results['health_histories']]
        avg_health_variation = np.mean(health_variations)
        
        return {
            'health_maintenance_efficiency': float(health_maintenance_efficiency),
            'health_improvement_efficiency': float(health_improvement_efficiency),
            'cost_per_health_point': float(total_cost / (avg_health + 1e-8)),
            'health_stability': float(1 / (1 + avg_health_variation)),  # 越高越稳定
            'performance_score': float((avg_health * 1000) / (total_cost + 1))  # 综合性能评分
        }

    def _calculate_entropy(self, probabilities):
        """计算信息熵"""
        return float(-np.sum([p * np.log2(p + 1e-8) for p in probabilities if p > 0]))
    ...
    def calculate_action_distribution(self, actions):
        """计算动作分布"""
        distribution = {}
        for action in range(4):  # action0-3
    ...
    def save_results(self, save_dir="paper/benchmark/simulation_results"):
        """保存仿真结果"""
...
def main():
    ...
    algorithm_name="cdt"
    # 查找最新的模型
    model_path = find_latest_model(algorithm_name=algorithm_name)  # can modify algorithm name
