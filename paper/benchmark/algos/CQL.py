...
class CQLDiscrete(nn.Module):
    """
    Conservative Q-Learning (CQL) for Discrete Action Spaces.
    Compatible with the interfaces of OneStepRL and CPQ.
    
    Algorithm:
    1. Standard DQN-style Bellman update (MSE loss).
    2. CQL Regularization: min sum(exp(Q(s,a))) - Q(s, a_data)
    """
    def __init__(
        self,
        state_dim: int,
    ...
    def compute_loss(self, batch):
        ...
        batch: dict with keys 'obs', 'actions', 'rewards', 'next_obs', 'dones', and optionally 'budget', 'next_budget'
        """
        ...
        total_loss = loss_bellman + self.cql_weight * loss_cql

        return total_loss, loss_bellman, loss_cql

    def update(self, batch, optimizer):
        """
        Updates Q network.
        """
        total_loss, loss_bellman, loss_cql = self.compute_loss(batch)
        ...
        return total_loss.item(), loss_bellman.item(), loss_cql.item()
